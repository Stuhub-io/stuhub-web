...
class PageService extends Client {
  ...
  public updateGeneralAccess(request: { pagePkID: number; body: UpdatePageGeneralAccessRequest }) {
    return fetcher<BaseResponse<Page>>(
      `${this.baseUrl}/v1/page-services/pages/${request.pagePkID}/general-access`,
      {
        method: 'PUT',
        headers: this.privateHeaders,
        body: JSON.stringify(request.body),
  ...
  public getPagePermissionRoles(pagePkID: number) {
    return fetcher<BaseResponse<PagePermissionRole[]>>(
      `${this.baseUrl}/v1/page-services/pages/${pagePkID}/roles`,
      {
        headers: this.privateHeaders,
      },
    )
  ...
  public updateUserPermissionRole({
    pagePkID,
    ...body
  }: {
    pagePkID: number
